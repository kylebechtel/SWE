...
public class Income {
	...
	double Tax(int income) {
		...
		if (income > 0 && income <= 20000)
			...
			System.out
					.println("Error Code: You appear to be homeless, no taxes for you");
		}
		return taxrate;
	}

	/*
	 * Gets the annual income from the client. If the client enters incorrect
	 * data or data in the incorrect format it returns 0.
	 */
	int getIncome() {
		while (true) {
			int income = 0;
				...
				while (true) {
					...
					System.out
							.println("I'm sorry that your annual wage is "
									+ income
									+ ", but I can't do anything with that information. Please try again.\n");
				}
			} catch (Exception e) {
				System.out
						.println("Are you homeless? please enter a proper input.  numbers only.\n");
			}
		}
