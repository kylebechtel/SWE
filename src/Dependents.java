...
public class Dependents {
	...
	double Tax(int numofdeps) {
		...
		if (numofdeps == 1)
			taxrate = .10;
		else if (numofdeps == 2 || numofdeps == 3)
			taxrate = .25;
		else if (numofdeps > 3)
			taxrate = .40;
		else if (numofdeps < 0)
			System.out
					.println("Umm, im not sure what you're doing with your kids, but you're doing it wrong.");

		return taxrate;
	}

	/*
	 * Gets the number of dependents from the client. If the client enters
	 * incorrect data or data in the incorrect format it returns 0.
	 */
	int getDependents() {
		while (true) {
			int Dependents = 0;
